...
```{block}
There is a second data set that you might be able to use as well. It is unclear how much the data overlaps but it appears to have almost all the same variables and quite different dates so you can increase your data.
...
```{block}
Not a lot of detail on the layout which could slow your development.
I am glad you are asking question but I would suggest more sophisticated ones you can answer with this data.

Try these two:
1. Can you predict the price of a shoe based on various variables.
2. Given a model for 1, What shoes are overprice or under priced compared to their equivalents
...
```{block}
I would rather see distributions statistics instead of just summary statistics
```
...
```{block}
Recommend Box plots over bar plots to see the distributions of prices based on categories.
Then allow the users to run a t.test or AOV to see if there is a difference in teh aveage price based on the choices.
```
...
```{block}
Suggest you create a tab to answer the questions I posed above. Allow users to select up to five variables for a linear model where they can chose to log variables, if appropriate, to build a linear model where the response variable is price and the others are explanatory variables they can select.

  Allow them to chose to see plots of the residuals from the model at least residuals vs fitted and qq plot for normality
